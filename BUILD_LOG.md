...
### 2026-06-29 — Brand identity for printed documents + AI-read autofill
- **Brand layer** `src/shared/brand.ts`: a `Brand` type + `GLORIA_BRAND` (the official Gloria Jean's guidelines — **GJ Orange #EF7D00 / GJ Black / White**, Tertiary Bone surface; Gotham → **Montserrat** webfont stand-in as Gotham is licensed; logo + company + banking). Exposes `brandCss` (palette vars + shared page/header/table chrome), `brandHeader` (logo + company block + title), `brandFontLink`. One source of truth so every printed document follows the brand — and so a café's own brand can be swapped in later.
- **Rebranded** invoice / statement / payslip generators (`src/shared/*-html.ts`) to thin consumers of the brand layer: black table headers, orange accent rule + orange "hero" total/net blocks, bone panels. Generators take an optional `brand` arg (default GLORIA_BRAND) ready for per-brand profiles. Verified by rendering a sample payslip → on-brand (GJ wordmark, orange NET PAY, Montserrat).
- **AI-read autofill** (`documents.ts`): `applyExtraction` now derives the **destination** from the read (till slip/payout → cash-up, invoice → purchases, turnover → month) so review pre-selects it; row **description falls back to supplier**; and on Apply the document is **dropped into the file catalog** (`department='Documents'`, ingested) so AI-read slips show up in **Records** automatically.
- Sandbox rule recorded ([[gloria-sandbox-and-brand]]): for anything Gloria Jeans only use files inside the project folder; ignore outside brands (e.g. SOSO). **Next:** brand_profiles table + Settings UI to upload/edit a brand (productization) and extend the brand layer to Letters & Reports.

### 2026-06-29 — Royalty audit & fix (per-store rates) + stark Store/Head-Office themes
- **Audit (Benoni May 2026 + all stores):** read the actual royalty invoice xlsx files vs the system. Found the engine billed a **blanket 8% royalty + 2.5% marketing to every store**, but each store has its own deal — over-stating May by ~R20k. Confirmed stable across April + May. True rates (royalty/marketing): Benoni 8%/0, Gateway 8%/0, Point 8%/0, Florida 8%/2.5, **Oceans Mall 6%/2.5** (its invoice mislabels the line "8%" but the amount is 6%). Pavilion **closed Apr-2026** but still owes HO — balance preserved, not recomputed. See memory [[per-store-royalty-rates]].
- **Fix:** migration v26 adds `stores.marketing_rate`; `generateRoyalties` now uses per-store `royalty_rate` + `marketing_rate` (removed the hardcoded 2.5%); added `setStoreMarketingRate` + `royalty:setMarketingRate` IPC/preload (editable). Seeded the real rates and **regenerated all 329 royalty invoices** — May now ties to the source invoices to the cent (Benoni 18,984.00, Gateway 37,380.77, Point 3,556.16, Oceans 5,874.15, Florida 24,527.19). Royalties page helper text corrected.
- **Stark office themes:** tailwind `gloria.brown/accent/cream` now resolve to CSS vars (`rgb(var(--gj-*) / <alpha>)`) set per `data-office` (OfficeModeContext writes the attribute). **Store (Oceans Mall)** = espresso brown + GJ Orange; **Head Office (Franchise)** = charcoal slate + Tasty Teal. Sidebar gains a bold mode badge (STORE/HEAD OFFICE). The whole chrome flips on office switch; printed documents keep their own GJ brand (unaffected). Verified by render.
- **Still open (noted for next):** the Creditors/Debtors (ledger_items) page only holds Florida/Gateway/Pavilion from the old archive ingest — Benoni/Oceans/Point have none; converge the HO debtor view onto royalty_invoices so it's complete + reconciled.
